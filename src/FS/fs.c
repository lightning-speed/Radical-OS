...
void fs_init()
{
  fs = (char *)malloc(4096 * 30);
  createNativeFiles();
}
void createFile(char *fn)
{
  for (int i = fs_index; i <= fs_index + 4096; i++)
  {
    fs[i] = 0;
  }
...
{
  char *out = malloc(34); //32-bit name
  int loc = indexF * 4096;
  for (int i = 0; i < 32; i++)
  {
    out[i] = fs[loc + i];
...
void deleteFileAt(int ie)
{
  int loc = ie * 4096;
  int inr = 0;
  for (int i = loc; i < loc + 4096; i++)
  {
    fs[i] = 0;
  }
  for (int i = loc + 4096; i <= fs_index; i++)
  {
    fs[loc + inr] = fs[i];
    inr++;
  }
...
void rename(char *fileName, char *to)
{
  if (doesFileExists(to))
    return;
  for (int i = 0; i < file_index; i++)
  {
    if (equal(fileName, getFileNameAt(i)))
    {
      int ind = 4096 * i;
      for (int i = 0; i < 32; i++)
      {
